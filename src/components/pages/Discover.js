...
class Discover extends React.Component {
	...
	async deleteFavoriteFood(event, index) {
        const unfavoriteFood = {
            user_id: parseInt(localStorage.getItem('user_id')), // Mengkonversi user_id menjadi integer jika diperlukan
            food_id: String(this.state.foods.data[index].id_food), // Mengubah food_id menjadi string
        };
    
        const response = await api.delete('favourite', { data: unfavoriteFood }); // Menggunakan opsi "data" untuk mengirim payload dalam metode delete
	...
	render() {
		...
		return (
			<div>
				...
				<div
					className={`${
						this.state.showFeedback ? 'blur' : 'blur-none'
					} z-0 flex px-16 pt-8`}
				>
					<div className="w-1/3">
